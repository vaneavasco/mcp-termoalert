...
server.tool(
    ...
    "Retrieve detailed incident logs and real-time status updates for Bucharest central heating network via Termoficare’s online portal for a specific sector",
...
server.tool(
    ...
    "Retrieve detailed incident logs and real-time status updates for Bucharest central heating network via Termoficare’s online portal for a specific street",
    { street: z.string() },
    async ({ street }) => {
        const issues = await retrieveIssues();
        const filteredIssues = issues.filter((issue) =>
            issue.zonesAffected.some((zone) => zone.toLowerCase().includes(street.toLowerCase())),
        );
        return {
            content: [
                {
